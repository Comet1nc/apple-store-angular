...
export class HeaderComponent implements OnInit, OnDestroy {
  ...
  searchViewDesktopAnimationState: SearchViewAnimationStates =
    SearchViewAnimationStates.invisible;

  bagViewIsVisible: boolean = false;

  navBarIsVisible: boolean = true;
  navBarDesktopAnimationState: NavBarAnimationStates =
    NavBarAnimationStates.visible;
  ...
  editModeAnimationsState: EditModeAnimationsStates =
    EditModeAnimationsStates.unactive;
  ...
  ngOnInit(): void {
    this.headerService.onChangeHeaderPosition.subscribe(
  ...
  ChangeStateSideNav(header: HTMLDivElement) {
    ...
    if (this.sidenavIsOpen) {
      ...
      setTimeout(() => {
        //  waiting until animation ends, and making some stuff
        this.sidenavIsOpen = false;
        this.navBarIsVisible = false;
        this.mobileSearchViewIsVisible = false;
  ...
  ShowSearchView() {
    ...
    // Show search view
    this.ToggleDesktopSearchViewAnimationState(
      SearchViewAnimationStates.visible
    );
  }
  ...
  CloseSearchView() {
    ...
    if (this.desktopSearchViewIsVisible && !this.navBarIsVisible) {
      ...
      // Show navbar
      setTimeout(() => {
        // this delay is for beautiful sequence, not necessary
  ...
  ToggleDesktopSearchViewAnimationState(state: SearchViewAnimationStates) {
    ...
    if (
      ...
      this.searchViewDesktopAnimationState ===
        SearchViewAnimationStates.invisible
    ) {
  ...
  ToggleDesktopNavbarAnimationState(state: NavBarAnimationStates) {
    // if trying animate from invisible to visible, show element
    if (
      state === NavBarAnimationStates.visible &&
      this.navBarDesktopAnimationState === NavBarAnimationStates.invisible
    ) {
      this.navBarIsVisible = true;
    }
    setTimeout(() => {
